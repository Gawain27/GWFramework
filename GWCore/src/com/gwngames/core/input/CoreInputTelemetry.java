...
public class CoreInputTelemetry extends BaseComponent implements IInputTelemetry {
    ...
    @Override public void released(IInputIdentifier id, long frame) { if (history != null) history.release(id); }
    ...
    /* New: just forward to history if you want counts; safe if history doesn’t implement them. */
    @Override public void axis(IInputIdentifier id, float raw, float norm, long frame) {
        if (history != null) history.axis(id, raw, norm);
    }
    @Override public void touchDown(IInputIdentifier id, float x, float y, float p, long frame) {
        if (history != null) history.touchDown(id, x, y, p);
    }
    @Override public void touchDrag(IInputIdentifier id, float x, float y, float p, long frame) {
        if (history != null) history.touchDrag(id, x, y, p);
    }
    @Override public void touchUp(IInputIdentifier id, float x, float y, float p, long frame) {
